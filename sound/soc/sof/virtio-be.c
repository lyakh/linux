...
static void sbe_ipc_fe_not_reply_get(struct sof_vbe *vbe, int vq_idx)
{
	...
	/*
	 * endchain means: we have already processed this queue
...
/*
 * This function is to get the BE dai link for the UOS
 * It uses the dai_link name to find the BE dai link.
 * The Current dai_link name "vm_dai_link" is for the UOS,
 ...
 */
static struct snd_pcm_substream *sbe_get_substream(struct snd_sof_dev *sdev,
...
/*
 * FIXME - this function should only convert a compressed UOS PHY page table
				     ...
				     void *ipc_buf, void *reply_buf)
{
	...
	int direction, ret;
	struct snd_sof_pcm *spcm = snd_sof_find_spcm_comp(sdev, comp_id,
							  &direction);
	if (!spcm)
		return -ENODEV;
 ...
 * valid and needs to be handled, let's call hanle_vq_kick(). Called in a
 * thread context.
 */
...
/*
 * register vhm client with virtio - called from a misc device ioctl().
 * vhm use the client to handle the io access from FE, it is a BE's proxy to
 * talk to the driver
 */
int sof_vbe_register_client(struct sof_vbe *vbe)
{
	...
	/*
	 * dev_info, including io_range_start and io_range_len are supplied from
	 * the ioctl()
	 */
...
int sof_vbe_register(struct snd_sof_dev *sdev, struct sof_vbe **svbe)
{
	 ...
	 */
	vbe->comp_id_begin = SOF_VIRTIO_MAX_UOS_COMPS;
			      ...
			      struct sof_ipc_stream_posn *posn)
{
	 ...
	 */
	if (!sbe_fill_posn_vqbuf(vbe, vq, posn, &endchain)) {
	...
	if (endchain)
		/*
		 * ACRN: in the current ACRN API the "virtio_" prefix is
		 * absent. 'Driver has finished processing "available"
		 * chains and calling vq_relchain on each one. Driver
		 * used all the available chains.'
		 */
		virtio_vq_endchains(vq, 1);
