			     ...
			     size_t size)
{
	...
	ret = spi_write(to_spi_device(sdev->parent), sof_pdata->ipc_buf,
			aligned_size);
	if (ret < 0)
		dev_err(sdev->dev, "%s(): error: SPI write failed: %d\n",
			__func__, ret);
...
static irqreturn_t spi_irq_thread(int irq __maybe_unused, void *context)
{
	...
	if (sof_pdata->fw_loading) {
		...
		struct sof_ipc_cmd_hdr *hdr =
			(struct sof_ipc_cmd_hdr *)sof_pdata->ipc_buf;
		int ret;
...
static int spi_fw_run(struct snd_sof_dev *sdev)
{
	...
	if (ret < 0) {
		dev_err(sdev->dev, "error: failed sending MEM_WRITE IPC: %d\n",
			ret);
		goto free;
	}
	...
	if (IS_ERR(tfm)) {
		dev_err(sdev->dev,
			"error: unable to create SHA256 crypto context\n");
		ret = PTR_ERR(tfm);
	} else {
	...
	ret = spi_write(spi, hdr, sizeof(*hdr));
	if (ret < 0) {
		dev_err(sdev->dev, "error: failed sending LOAD IPC: %d\n", ret);
		goto free;
	}
	...
	for (data = sof_pdata->fw->data, size = sof_pdata->fw->size;
		...
		if (ret < 0) {
			dev_err(sdev->dev,
				"error: failed sending FW image: %d\n", ret);
			goto free;
		}
	}
	...
	ret = spi_write(spi, hdr, sizeof(*hdr));
	if (ret < 0)
		dev_err(sdev->dev, "error: failed sending EXEC IPC: %d\n", ret);
